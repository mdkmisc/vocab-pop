...
export default class SourceTargetSource extends Component {
  constructor(props) {
...
class STSReport extends Component {
  constructor(props) {
    super(props);
    this.state={};
  }
  ...
  fetch() {
    let {vocabulary_id, concept_codes} = this.props;
    console.log("get stuff for", vocabulary_id, concept_codes);
    let stream = new AppState.ApiStream({
      apiCall: 'codesToSource',
      params: {vocabulary_id, concept_codes},
      //meta: { statePath },
      //transformResults,
    });
    stream.subscribe((recs,stream)=>{
      ...
      this.setState({recs, sg});
    });
    //this.setState({stream});
  }
  render() {
    let {vocabulary_id, concept_codes, } = this.props;
    let {recs=[], sg} = this.state;
    return  <div>
